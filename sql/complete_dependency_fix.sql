-- Complete script to create all required data with all dependencies resolved

-- Install pgcrypto extension if not already installed (requires superuser privileges)
-- This needs to be run separately as superuser: CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- First, ensure we have role records (this is the top of the dependency chain)
INSERT INTO role (id_role, libelle, cree_le, modifie_le)
SELECT '1', 'admin', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM role WHERE id_role = '1');

INSERT INTO role (id_role, libelle, cree_le, modifie_le)
SELECT '2', 'institution', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM role WHERE id_role = '2');

INSERT INTO role (id_role, libelle, cree_le, modifie_le)
SELECT '3', 'superviseur', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM role WHERE id_role = '3');

-- Next, create utilisateur records (depends on role) with bcrypt hashed passwords
-- These are pre-hashed with BCrypt to match Spring Security's PasswordEncoder format
INSERT INTO utilisateur (id_utilisateur, email, mot_de_passe, nom, prenom, cree_le, modifie_le, id_role)
SELECT 'USR001', 'admin@lbcft.mg', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Administrateur', 'Système', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '1'
WHERE NOT EXISTS (SELECT 1 FROM utilisateur WHERE id_utilisateur = 'USR001');

INSERT INTO utilisateur (id_utilisateur, email, mot_de_passe, nom, prenom, cree_le, modifie_le, id_role)
SELECT 'USR002', 'contact@bnm.mg', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Directeur', 'BNM', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '2'
WHERE NOT EXISTS (SELECT 1 FROM utilisateur WHERE id_utilisateur = 'USR002');

INSERT INTO utilisateur (id_utilisateur, email, mot_de_passe, nom, prenom, cree_le, modifie_le, id_role)
SELECT 'USR003', 'info@sogem.mg', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Gestionnaire', 'SOGEM', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '2'
WHERE NOT EXISTS (SELECT 1 FROM utilisateur WHERE id_utilisateur = 'USR003');

INSERT INTO utilisateur (id_utilisateur, email, mot_de_passe, nom, prenom, cree_le, modifie_le, id_role)
SELECT 'USR004', 'contact@caamada.mg', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Avocat', 'Principal', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '2'
WHERE NOT EXISTS (SELECT 1 FROM utilisateur WHERE id_utilisateur = 'USR004');

-- Update any existing records that still have example passwords
UPDATE utilisateur
SET mot_de_passe = '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi'
WHERE mot_de_passe = '$2y$10$example_hashed_password';
...
WHERE (denomination_sociale IS NULL OR denomination_sociale = '') AND nom IS NOT NULL;

-- Insert institution records (depends on utilisateur)
...
-- Insert EPNFD sectors if they don't exist (independent reference table)
...
-- Insert activity details if they don't exist (independent reference table)
...
-- Finally insert the questionnaire responses (depends on institution, epnfd_sectors, and activity_details)
